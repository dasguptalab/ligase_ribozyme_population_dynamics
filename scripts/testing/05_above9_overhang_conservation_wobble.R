...
for (run_num in min(round_list):max(round_list)) {
  ...
  for (bin_index in 1:data_length) {
    ...
    if (cur_type == "T") { # total
      ...
      # add overhang complement counts
      complement_counts$counts[bin_index] <- sum(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "no" & complement_data_subset$wobble == "no" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num, "counts"])
      # add overhang complement unique counts
      complement_counts$counts_unique[bin_index] <- nrow(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "no" & complement_data_subset$wobble == "no" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num,])
    }else if (cur_type == "G") { # gaped
      # add overhang complement counts
      complement_counts$counts[bin_index] <- sum(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "yes" & complement_data_subset$wobble == "no" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num, "counts"])
      # add overhang complement unique counts
      complement_counts$counts_unique[bin_index] <- nrow(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "yes" & complement_data_subset$wobble == "no" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num,])
    }else if (cur_type == "W") { # gaped
      # add overhang complement counts
      complement_counts$counts[bin_index] <- sum(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "no" & complement_data_subset$wobble == "yes" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num, "counts"])
      # add overhang complement unique counts
      complement_counts$counts_unique[bin_index] <- nrow(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "no" & complement_data_subset$wobble == "yes" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num,])
    }else if (cur_type == "WG") { # gaped
      # add overhang complement counts
      complement_counts$counts[bin_index] <- sum(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "yes" & complement_data_subset$wobble == "yes" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num, "counts"])
      # add overhang complement unique counts
      complement_counts$counts_unique[bin_index] <- nrow(complement_data_subset[complement_data_subset$identity == cur_identity & complement_data_subset$gap == "yes" & complement_data_subset$wobble == "yes" & complement_data_subset$run_name == run_num & complement_data_subset$counts_run_name == run_num,])
    }
...
complement_counts_wobble <- complement_counts_out[complement_counts_out$type == "W",]
complement_counts_wobble_gap <- complement_counts_out[complement_counts_out$type == "WG",]
...
# create line plot of gaped overhang identity percent
base_counts_plot <- ggplot(data=complement_counts_wobble, aes(x=as.character(run_name), y=perc_abundance_unique, group=identity, color=identity_color))+
  geom_line() +
  geom_point() +
  theme_bw() +
  scale_color_identity(name = "Identity", labels = complement_counts_wobble$identity, breaks = complement_counts_wobble$identity_color, guide = "legend") +
  ylab("Percent Abundance") +
  xlab("Round Number")
# save the plot
exportFile <- paste(out_dir, "/above9_overhang_percent_abundance_unique_wobble.png", sep = "")
png(exportFile, units="in", width=5, height=5, res=300)
print(base_counts_plot)
dev.off()

# create line plot of gaped overhang identity percent
base_counts_plot <- ggplot(data=complement_counts_wobble_gap, aes(x=as.character(run_name), y=perc_abundance_unique, group=identity, color=identity_color))+
  geom_line() +
  geom_point() +
  theme_bw() +
  scale_color_identity(name = "Identity", labels = complement_counts_wobble_gap$identity, breaks = complement_counts_wobble_gap$identity_color, guide = "legend") +
  ylab("Percent Abundance") +
  xlab("Round Number")
# save the plot
exportFile <- paste(out_dir, "/above9_overhang_percent_abundance_unique_wobble_gaped.png", sep = "")
png(exportFile, units="in", width=5, height=5, res=300)
print(base_counts_plot)
dev.off()

...
out_dir <- "/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/plots/08_above9_overhang_conservation"
...
# create heatmap of overhang identity counts
base_counts_plot <- ggplot(data = overhang_counts_out, aes(reorder(as.character(run_ID), run_ID), reorder(as.character(identity), identity), fill = counts_na)) + 
  theme_bw() +
  ...
  scale_fill_gradient2(name = "Counts",
                       low = safe_colors[3],
                       mid = safe_colors[4],
                       high = safe_colors[5],

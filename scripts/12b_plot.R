...
r8_peaks_identity <- read.csv("/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/09_cluster_identity/07b_clustered/r8_S8_L001_formatted_above9_cluster_peaks_identity_table.csv")
peaks_identity <- read.csv("/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/09_cluster_identity/07b_clustered/peaks_identity_table.csv")
...
r8_seqs_family <- read.csv("/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/10_cluster_families/07b_clustered/r8_S8_L001_cluster_families_table.csv")
r8_peaks_family <- read.csv("/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/10_cluster_families/07b_clustered/r8_S8_L001_cluster_peaks_families_table.csv")
...
subset_abundances <- head(cluster_abundances, n=10)
subset_colors <- head(safe_colors, n=10)
subset_cluster_list <- unique(subset_abundances$cluster_ID)
subset_cluster_list_out <- subset_cluster_list+1
ggplot(data=cluster_abundances, aes(x=run_name, y=frac_abundance, group=cluster_ID, color=cluster_color))+
  geom_line() +
  geom_point() +
  theme_bw() +
  scale_color_identity(labels = subset_cluster_list_out, breaks = safe_colors, name = "Family", guide = "legend") +

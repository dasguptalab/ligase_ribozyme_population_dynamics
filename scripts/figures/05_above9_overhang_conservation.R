...
#expected_overhang <- c("T","G","C","G","G")
expected_overhang <- c("TGCGG")
...
overhang_matrix <- seqs_matrix[,16:20]
...
for (seq_num in 1:data_length) {
  ...
  num_match <- mapply(
    ...
    strsplit(overhang_data$overhang[seq_num], ''), 
    strsplit(expected_overhang, '')
  )
  # add percent identity to expected overhang sequence
  overhang_data$identity[seq_num] <- 100*num_match/frag_length
}
  
# vector of identity bins
identity_bins <- seq(0, 100, by = 20)
...
base_counts_plot <- ggplot(data=overhang_counts_out, aes(x=as.character(run_ID), y=perc_abundance, group=identity, color=identity_color))+
  ...
  geom_point() +
  theme_bw() +
  scale_color_identity(name = "Identity", labels = c(20, 40, 60, 80, 100), breaks = safe_colors[1:5], guide = "legend") +
...
overhang_counts_subset <- na.omit(overhang_counts_out[overhang_counts_out$identity >= 60,])
...
base_counts_plot <- ggplot(data=overhang_counts_subset, aes(x=as.character(run_ID), y=perc_abundance, group=identity, color=identity_color))+
  ...
  xlab("Round Number")
# save the plot
exportFile <- paste(out_dir, "/above9_overhang_percent_abundance_lines_atLeast60.png", sep = "")
png(exportFile, units="in", width=5, height=5, res=300)
print(base_counts_plot)
dev.off()

# subset the overhang counts by identity
overhang_counts_subset <- na.omit(overhang_counts_out[overhang_counts_out$identity >= 80,])

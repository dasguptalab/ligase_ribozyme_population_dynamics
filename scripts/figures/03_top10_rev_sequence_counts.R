...
out_dir <- "/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/figures/03_top10_rev_sequence_counts"
...
for (run_num in 1:8) {
  ...
  # save the plot
  exportFile <- paste(out_dir, "/r", run_num, "_top10_rev_sequence_log_counts.png", sep = "")
...
# save the plot
exportFile <- paste(out_dir, "/top10_rev_sequence_log_counts.png", sep = "")
png(exportFile, units="in", width=40, height=10, res=300)
print(grid_log_counts_plot)
dev.off()

# export plotting data
write.csv(seqs_counts, file = paste(out_dir, "/top10_rev_sequence_counts.csv", sep = ""), row.names = FALSE, quote = FALSE)
